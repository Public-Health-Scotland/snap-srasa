...
# *WRITE LOOP to do this site-by-site*
...
# Intuitive
int_gri <- int_minimal %>% 
  filter(hospital_name == "Glasgow Royal Infirmary") %>% 
...
non_ras_smr <- smr_data %>% 
  filter(op_mth == "2025-11-01" & 
           ras_proc == FALSE &
           hospital_name == "Glasgow Royal Infirmary") %>% 
  mutate(main_op_type = replace_when(main_op_type, 
                                     main_op_type == "Abdominal hysterectomy" | 
                                       main_op_type == "Vaginal hysterectomy" ~ "Hysterectomy"),
         main_op_type = replace_when(main_op_type, 
...
unmatched_records_int <- failed_int_info %>% 
  ...
  write_csv(paste0("../testing/unmatched_records_int_", hb_acronym, "_", Sys.Date(), ".csv"))
...
unmatched_records_smr <- failed_smr_info %>% 
  filter(match_found == "no match") %>% 
  write_csv(paste0("../testing/unmatched_records_smr_", hb_acronym, "_", Sys.Date(), ".csv"))
...
join_results <- rbind(join_b_data, join_c_data, join_d_data, join_e_data, join_f_data) %>% 
  ...
  write_csv(paste0("../testing/join_result_detail_", hb_acronym, "_", Sys.Date(), ".csv"))
