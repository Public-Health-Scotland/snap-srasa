...
append_lookups <- function(df, which_lookups = c("all", "postcode", "urban_rural", "hospital", "specialty")){
  ...
  cli_progress_step("Appending lookup files...")
  ...
  ras_lookup_data <- df %>% 
    ...
    relocate(age_group, .after = age_in_years)
  ...
  ras_lookup_data <- ras_lookup_data %>% 
    ...
    relocate(simd_quintile, res_health_board, .after = postcode)
}
  ...
  ras_lookup_data <- ras_lookup_data %>% 
    ...
    relocate(urban_rural_6, .after = simd_quintile)
  }
  ...
  ras_lookup_data <- ras_lookup_data %>% 
    ...
    relocate(hospital_name, hosp_health_board, hosp_has_robot, .after = location)
  }
